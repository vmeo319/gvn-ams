...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    if (!accessToken || !refreshToken) {
      setErrorMsg('This password reset link is missing or malformed.')
      setStatus('error')
      return
    }
    ...
    <main className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full bg-slate-900 border border-red-900/30 rounded-3xl p-8 shadow-2xl shadow-red-950/40 space-y-6">
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <p className="text-slate-400 text-xs tracking-wider uppercase pt-1">Reset Your Password</p>
        </div>
        ...
        {status === 'error' && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-xs p-3 rounded-xl text-center font-medium">
            {errorMsg || 'This reset link is invalid or has expired. Request a new one from the sign in page.'}
          </div>
        )}
