...
export default function SignUp() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const res = await requestAccount({ firstName, lastName, email, password })
    setLoading(false)

    if (!res.success) {
      setErrorMsg(res.error || 'Could not submit your request.')
      return
    }
        ...
        {submitted ? (
          <div className="space-y-4 text-center">
            <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm p-4 rounded-xl">
              Request submitted! A coach will review it and approve your account. You'll be able to
              sign in once that happens.
            </div>
            <Link href="/" className="block text-center text-xs text-slate-400 hover:text-white transition">
              Back to sign in
            </Link>
          </div>
        ) : (
          <>
            <p className="text-slate-400 text-xs text-center leading-relaxed">
              Enter your name, email, and a password. A coach will review your request and approve
              your account — this keeps the roster accurate.
            </p>
